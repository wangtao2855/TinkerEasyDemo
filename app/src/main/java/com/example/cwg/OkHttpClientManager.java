...
public class OkHttpClientManager {
    ...
    public OkHttpClient.Builder getmOkHttpClient() {
    ...
    private OkHttpClientManager() {
        ...
        mOkHttpClient.connectTimeout(10, TimeUnit.SECONDS);// 连接超时
        mOkHttpClient.writeTimeout(10, TimeUnit.SECONDS);//10m秒写入超时
        mOkHttpClient.readTimeout(10, TimeUnit.SECONDS);// 读取超时
        mDelivery = new Handler(Looper.getMainLooper());
    }

    public static void setTimeOutLong() {
        mInstance.mOkHttpClient.connectTimeout(10, TimeUnit.SECONDS);// 连接超时
        mInstance.mOkHttpClient.readTimeout(10, TimeUnit.SECONDS);// 读取超时
    }
    ...
    private Response _post(String url, Param... params) throws IOException {
        ...
        Response response = mOkHttpClient.build().newCall(request).execute();
        return response;
    }
    ...
    private void _downloadAsyn(final String url, final String destFileDir, final StringCallback callback) {
        final Request request = new Request.Builder().url(url).build();
        final Call call = mOkHttpClient.build().newCall(request);
    ...
    public void getAsyn(String url, StringCallback callback) {
    ...
    public void setCertificates(InputStream... certificates) {
        try {
            ...
            mOkHttpClient.build().sslSocketFactory();
        } catch (Exception e) {
            e.printStackTrace();
        }
